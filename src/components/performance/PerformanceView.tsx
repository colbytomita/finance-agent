...
import {
  ...
  type DateRange,
} from "@/services/performanceFilter";
import type { PerformanceReport } from "@/services/signalPerformanceCore";
import type { ClosedTradeInput, JournalInput, TradeStats } from "@/services/tradePerformance";
import { fmtDateTime, fmtMoney } from "@/lib/format";
import { Pct } from "@/components/badges";
import { RunBacktestButton } from "@/components/SignalPerformance";
import { DateRangeSlider } from "./DateRangeSlider";

// Client shell for /performance. The backtest is far too slow to re-run per date
// range (it refetches bars over the network), so the report ships with its raw
// per-event rows and this component re-pools them locally as the slider moves —
// see performanceFilter.ts. At the full range it renders the STORED aggregates
// untouched, so an unfiltered page is exactly what the backtest produced.
...
export interface PerformanceViewProps {
  ...
  /** All-time realized stats, rendered as-is when the range isn't narrowed. */
  trades: TradeStats;
  closedTrades: ClosedTradeInput[];
  journal: JournalInput[];
}

export function PerformanceView({ report, trades, closedTrades, journal }: PerformanceViewProps) {
  const domain = useMemo(
    () =>
      performanceDateDomain(
        report?.events,
        closedTrades.map((t) => t.closedAt?.slice(0, 10) ?? "").filter(Boolean),
      ),
    [report?.events, closedTrades],
  );
  // Without stored per-event rows (a report from before this feature) there is
  // nothing to re-pool, so the page stays exactly as it was until the next run.
  const canFilter = report?.events != null && domain != null;
  const [range, setRange] = useState<DateRange | null>(null);
  const effective = canFilter ? (range ?? domain) : null;
  const isFullRange =
    effective == null || domain == null || (effective.from === domain.from && effective.to === domain.to);

  const filtered = useMemo(
    () => (report && effective && !isFullRange ? filterPerformance(report, effective) : null),
    [report, effective, isFullRange],
  );

  const score = filtered?.score ?? report?.score;
  const picks = filtered?.picks ?? report?.picks;
  const setups = filtered?.setups ?? report?.setups;
  const tradeStats =
    effective && !isFullRange ? filterClosedTrades(closedTrades, journal, effective) : trades;

  const eventCount =
    (score?.analyzed ?? 0) + (picks?.analyzed ?? 0) + (setups?.totalSetups ?? 0) + tradeStats.closed;
        ...
        {tradeStats.closed === 0 ? (
          <p className="py-4 text-center text-sm text-zinc-500">
            {isFullRange
              ...
              : "No trades closed inside the selected date range."}
          </p>
        ) : (
          <>
            <p className="text-[11px] text-zinc-500">
              {tradeStats.closed} closed · {tradeStats.wins}W / {tradeStats.losses}L
              {tradeStats.breakeven ? ` / ${tradeStats.breakeven} BE` : ""}
            </p>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
              <Stat label="Win rate" value={pct(tradeStats.winRate, 0)} cls={tradeStats.winRate != null && tradeStats.winRate >= 50 ? "pos" : "text-zinc-200"} />
              <Stat label="Avg return / trade" value={pct(tradeStats.avgReturnPct)} cls={(tradeStats.avgReturnPct ?? 0) > 0 ? "pos" : (tradeStats.avgReturnPct ?? 0) < 0 ? "neg" : "text-zinc-200"} />
              <Stat label="Avg R multiple" value={tradeStats.avgRMultiple == null ? "—" : `${tradeStats.avgRMultiple.toFixed(2)}R`} cls={(tradeStats.avgRMultiple ?? 0) > 0 ? "pos" : (tradeStats.avgRMultiple ?? 0) < 0 ? "neg" : "text-zinc-200"} />
              <Stat label="Profit factor" value={tradeStats.profitFactor == null ? "—" : tradeStats.profitFactor.toFixed(2)} />
              <Stat label="Avg win" value={pct(tradeStats.avgWinPct)} cls="pos" />
              <Stat label="Avg loss" value={pct(tradeStats.avgLossPct)} cls="neg" />
              <Stat label="Avg hold" value={tradeStats.avgHoldingDays == null ? "—" : `${tradeStats.avgHoldingDays.toFixed(1)}d`} />
              <Stat label="Total P/L" value={tradeStats.totalPnl == null ? "—" : fmtMoney(tradeStats.totalPnl)} cls={(tradeStats.totalPnl ?? 0) > 0 ? "pos" : (tradeStats.totalPnl ?? 0) < 0 ? "neg" : "text-zinc-200"} />
              <Stat label="Best / worst" value={`${pct(tradeStats.bestPct, 0)} / ${pct(tradeStats.worstPct, 0)}`} />
              {tradeStats.thesisPlayedOutRate != null && (
                <Stat label="Thesis played out" value={pct(tradeStats.thesisPlayedOutRate, 0)} />
              )}
            </div>
          </>
        )}
