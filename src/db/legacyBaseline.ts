...
// FROZEN pre-drizzle-kit schema (roadmap #5). Databases created before the
// switch to generated migrations were built by this DDL plus the additive
// ALTERs below. applyLegacyBaseline() runs once, the first time such a
// database is opened by the new code, to bring any older copy up to the exact
// shape of migration 0000 — after which the drizzle journal takes over and
// this file is never consulted again. Do NOT edit this schema; new schema
// changes go in src/db/schema.ts + `npm run db:generate`.
...
/** Bring a pre-migration database to the migration-0000 shape (idempotent). */
export function applyLegacyBaseline(sqlite: InstanceType<typeof Database>): void {
  sqlite.exec(LEGACY_DDL);
  for (const stmt of LEGACY_ALTERS) {
    try {
      sqlite.exec(stmt);
    } catch {
