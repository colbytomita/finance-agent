...
**Signal Performance — "does the score actually work?" (at `/performance`).** A calibration backtest of the app's *own* output. Because a `stock_scores` row is appended on every recompute, the app already holds a time-series of every score it has produced; this turns each into an event and **reuses the Catalyst Edge event-study engine** to measure that ticker's forward return vs SPY over the next 1 / 5 / 20 trading days, pooled **by recommendation band**. If the score is calibrated, higher bands should show higher forward abnormal returns — surfaced as a per-band table plus a one-line verdict (*improves / mixed / inverts*). It de-duplicates to one score per ticker per day, benchmarks against a forward-refreshed SPY, and clearly explains when there isn't yet enough matured data (e.g., daily bars lagging the newest scores). Historical correlation across past calls — not a prediction, not advice.
...
| `npm test` | vitest suite (scoring, buy zones, risk, exits, parsers, event study, ingestion, edge, research universe, earnings, CIK resolution, sector scout, signal performance — 229 tests) |
| `npm run typecheck` | strict TypeScript check |
| `npm run db:seed` | optional demo data — not required; the app is designed to run on your real data |

## Architecture

- **`src/services/*`** — the engine. Market data & orchestration (`marketData`, `alpaca`, `yahooFinanceBrowser`), analytics (`indicators`, `buyZone`, `scoring`, `tradeScoring`, `setupDetection`, `riskManagement`), catalysts & research (`catalysts`, `researchAgent`, `alerts`), discovery (`discoveryAgent`, `sectorScout` — industry-targeted scan: LLM/curated theme→ticker expansion, validation, scoring, and per-pick briefs), signal-performance backtest (`signalPerformance` — reuses the event-study engine to calibrate the app's own scores by recommendation band), earnings surprise (`earnings` — model, calibrated surprise→impact mapping, Yahoo fetch), and the Catalyst Edge stack: `eventStudy` (pure math), `entityMentions` (CRUD + per-entity analysis with bar backfill), `eventExtraction` (Haiku + rule-based fallback), `eventIngestion` (sources → extract → dedupe → persist), `catalystEdge` (edge → catalysts → scoring), and `sources/*` (SEC EDGAR, GDELT, IR RSS connectors, feed parsers, company→ticker map, and SEC CIK→ticker map).
- **`src/db/*`** — Drizzle schema + SQLite DDL (Postgres-ready). Tables include portfolio holdings, watchlist, price bars, snapshots, drawdowns, catalysts, stock/trade scores, setups, journal, alerts, app settings, agent candidates, `entity_mentions`, and the Sector Scout run log + picks (`sector_scans`, `sector_scout_picks`).
- **`src/app/*`** — App-Router pages (Summary, Portfolio, Watchlist, Agent Picks, Sector Scout, Signal Performance, Swing Trading, Catalysts, Catalyst Edge, Research Universe, Settings, per-stock detail) and the JSON API routes behind them.
...
`npm run typecheck` (strict) and `npm test` (229 vitest tests) both run clean. The test suite focuses on pure logic — scoring, buy zones, risk math, exits, indicator/feed parsers, the event-study windows and aggregation, ticker resolution, LLM-response parsing with fallbacks, the edge-impact mapping, the research-universe dataset shape, Sector Scout's ticker-list parsing / theme matching / rule-based brief, and the Signal Performance bucketing + calibration verdict — using synthetic fixtures so no network or database is required.
