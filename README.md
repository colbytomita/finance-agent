...
- **`src/app/*`** — App-Router pages (Summary, Portfolio, Watchlist, Agent Picks, Sector Scout, Swing Trading, Catalysts, Catalyst Edge, Research Universe, Settings, per-stock detail) and the JSON API routes behind them.
...
`npm run typecheck` (strict) and `npm test` (222 vitest tests) both run clean. The test suite focuses on pure logic — scoring, buy zones, risk math, exits, indicator/feed parsers, the event-study windows and aggregation, ticker resolution, LLM-response parsing with fallbacks, the edge-impact mapping, the research-universe dataset shape, and Sector Scout's ticker-list parsing / theme matching / rule-based brief — using synthetic fixtures so no network or database is required.
